...
const Resellers = (() => {
  ...
  // ── State ─────────────────────────────────────────────────────────────────
  let _registryData  = [];   // full reseller objects from API
  ...
  const _api = (path, body) => {
    if (body) return fetch(path, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminKey: Store.adminKey, ...body }) }).then(r => r.json());
  ...
  const setTab = (tab, btn) => {
    ...
    document.getElementById('reseller-tab-registry').style.display    = tab === 'registry'    ? '' : 'none';
    document.getElementById('reseller-tab-performance').style.display = tab === 'performance' ? '' : 'none';
    _renderActiveTab();
  };
  ...
  const _renderRegistry = () => {
    ...
    if (!_registryData.length) {
      wrap.innerHTML = '<div class="empty">No resellers yet. Click <strong>＋ Add Reseller</strong> to create the first one.<br><span style="font-size:.75rem;color:var(--muted2)">Resellers created here auto-appear in the link generation dropdown.</span></div>';
      return;
    }
    wrap.innerHTML = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:.8rem">' +
      _registryData.map(r => _buildRegistryCard(r)).join('') + '</div>';
  };

  const _buildRegistryCard = (r) => {
    ...
    const eid   = esc(r.id); const en = esc(r.name);
    ...
    return `<div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:1rem 1.1rem;${susp ? 'opacity:.72;' : ''}">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.5rem;margin-bottom:.75rem">
        <div style="flex:1;min-width:0">
          ...
          ${tOnly ? '<div style="font-size:.62rem;color:var(--warn);margin-top:.2rem">⚠ Token-only — not in registry</div>' : ''}
        </div>
      ...
      ${r.wechat || r.country ? `<div style="font-size:.7rem;color:var(--muted);margin-bottom:.65rem">${r.wechat ? '💬 ' + esc(r.wechat) : ''}${r.country ? (r.wechat ? '  ·  ' : '') + '🌍 ' + esc(r.country) : ''}</div>` : ''}
      ...
      ${r.notes ? `<div style="font-size:.7rem;color:var(--muted);background:#f8fafc;border-radius:6px;padding:.4rem .6rem;margin-bottom:.75rem;line-height:1.5">${esc(r.notes)}</div>` : ''}
      <div style="display:flex;gap:.3rem;flex-wrap:wrap;border-top:1px solid var(--border);padding-top:.65rem">
        ${!tOnly
          ...
          : `<button class="btn btn-primary btn-sm" onclick="Resellers.addToRegistry('${eid}','${en}')">＋ Add to Registry</button>`}
        <button class="btn btn-ghost-blue btn-sm" onclick="Resellers.filterByReseller('${eid}')">🔍 Links</button>
        <button class="btn btn-outline btn-sm" onclick="navigator.clipboard.writeText('${eid}').then(()=>alert('✓ '+`${'${eid}'}` ))">📋 ID</button>
      </div>
    </div>`;
  };
  ...
  const _renderPerformance = () => {
    ...
    resSales.forEach(([, t]) => {
      ...
      byR[rid].total += (t.price || 0); byR[rid].count++; byR[rid].items.push(t);
    });

    const sorted = Object.entries(byR).sort((a, b) => b[1].total - a[1].total);
    if (!sorted.length) { wrap.innerHTML = '<div class="empty">No reseller sales yet.</div>'; return; }
      ...
      return `<div class="reseller-card">
        <div class="reseller-header">
          <div style="display:flex;align-items:flex-start;gap:.8rem">
            <div style="width:28px;height:28px;border-radius:50%;background:${rc}20;border:1.5px solid ${rc};display:flex;align-items:center;justify-content:center;font-size:.72rem;font-weight:700;color:${rc};flex-shrink:0">${i + 1}</div>
            <div>
  ...
  const openModal = (id) => {
    ...
    document.getElementById('rm-id').disabled = !!id;
    document.getElementById('reseller-modal-title').textContent = id ? 'Edit Reseller' : 'Add Reseller';

    const clear = ['rm-id','rm-name','rm-commission','rm-wechat','rm-country','rm-notes'];
    clear.forEach(fid => { const el = document.getElementById(fid); if (el) el.value = ''; });

    if (id) {
      const r = _registryData.find(r => r.id === id);
      if (r) {
        document.getElementById('rm-id').value         = r.id;
        document.getElementById('rm-name').value       = r.name;
        document.getElementById('rm-commission').value = r.commissionRate || '';
        document.getElementById('rm-wechat').value     = r.wechat || '';
        document.getElementById('rm-country').value    = r.country || '';
  ...
  const saveModal = async () => {
    ...
    const allowedProducts = [...document.querySelectorAll('.rm-prod-chk:checked')].map(c => c.value);
    ...
    try {
      ...
      if (d && d.success) { closeModal(); await render(); }
      else showErr(d.error || 'Save failed.');
    } catch { showErr('Network error.'); }

    saveBtn.disabled = false; saveBtn.textContent = 'Save Reseller';
  };
  ...
  const deleteReseller = async (id) => {
    if (!confirm(`Remove "${id}" from the registry?\n\nHistorical sales data is kept — only the registry entry and dropdown listing are removed.`)) return;
  ...
  const _populateDropdown = () => {
    ...
    sel.innerHTML = '<option value="">— No reseller (direct sale) —</option>' +
      active.map(r =>
        `<option value="${esc(r.id)}" data-name="${esc(r.name)}" data-rate="${r.commissionRate || 0}">${esc(r.name)} (${esc(r.id)})</option>`
      ).join('');
  ...
  return { render, setTab, openModal, closeModal, saveModal, suspend, deleteReseller, addToRegistry, copyId, filterByReseller, getSelectedReseller, populateDropdown: _populateDropdown };
})();
